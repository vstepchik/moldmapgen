# source: https://github.com/peixian/kingfisher
...
class Atlas(object):
    ...
    Implements Fortune's Algorithm in python.
    1. Takes in a 2D numpy array (or generates points with given boundaries)
    2. Creates a tuple of two graphs (with networkx) representing the Delaunay triangulation
    3. Relaxes the points through Lloyd's Algorithm
    4. Returns a Voronoi diagram
    """
    ...
    def __init__(self, points=np.array([]), dimensions=(None, None), granularity=None):
        ...
        if len(points) == 0 and dimensions == (None, None):
            print('You can\'t have both points and dimensions be empty.')
            return
        if len(points) == 0 and dimensions is not None and granularity is None:
    ...
    def generate_voronoi(self):
        ...
        Filters viable regions and stashes them in filtered_regions,
        see https://stackoverflow.com/questions/28665491/getting-a-bounded-polygon-coordinates-from-voronoi-cells
        ...
        for region in self.vor.regions:
            ...
            for index in region:
                if index == -1:
                    ...
                    if not (self.bounding_region[0] - eps <= x <= self.bounding_region[1] + eps and
                            self.bounding_region[2] - eps <= y <= self.bounding_region[3] + eps):
                        flag = False
                        break
            if region != [] and flag:
    ...
    def _region_centroid(self, vertices):
        ...
        :type vertices: numpy array of vertices from the scipy.spatial.Voronoi.regions
            (e.g. vor.vertices[region + [region[0]], :])
